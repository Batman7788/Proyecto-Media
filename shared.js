/* ─── DATOS COMPARTIDOS ─── */
window.__schoolBoxState = window.__schoolBoxState || {
    enceresPorSalon: null,
    historial: []
};

/* Inventario de enceres por salón (en memoria del navegador) */
function obtenerEnceres() {
    var state = window.__schoolBoxState;
    if (!state.enceresPorSalon) {
        var data = {};
        for (var s = 1; s <= 10; s++) {
            data[s] = [
        ...
        state.enceresPorSalon = data;
    }
    return state.enceresPorSalon;
}

function guardarEnceres(data) {
    window.__schoolBoxState.enceresPorSalon = data;
}

/* Historial (en memoria del navegador) */
function obtenerHistorial() {
    return window.__schoolBoxState.historial || [];
}

function guardarHistorial(h) {
    window.__schoolBoxState.historial = h;
}
...
function validarRol(rolEsperado) {
    var rol = document.body.getAttribute('data-rol') || '';
    if (rol !== rolEsperado) {
        window.location.href = 'registro.php';
        return false;
    }
